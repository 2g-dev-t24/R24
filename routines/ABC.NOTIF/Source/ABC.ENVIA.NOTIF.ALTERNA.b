* @ValidationCode : MjotMTM2MzYzOTU4OkNwMTI1MjoxNzcyNDE2MDgzMjM0Okx1aXMgQ2FwcmE6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjRfU1AxLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 01 Mar 2026 22:48:03
...
**************
    ...
    LOCATE 'JAR_PATH' IN Y.LIST.PARAMS SETTING Y.POS THEN
        Y.JAR.PATH = Y.LIST.VALUES<Y.POS>
    END
...
**************
    ...
    MENSAJE = ID.SMS.EMAIL:", str_path: ":str_path
    GOSUB BITACORA.ALT
    
    MENSAJE = ID.SMS.EMAIL:", str_filename: ":str_filename
    GOSUB BITACORA.ALT
    
    MENSAJE = ID.SMS.EMAIL:", TEMP.FILE: ":TEMP.FILE
    GOSUB BITACORA.ALT
...
* Armo Archivo
    Y.SHELL = Y.SHELL : "cd ":Y.JAR.PATH:AbcNotif.getYSalto()
    Y.SHELL = Y.SHELL : Y.CADENA : AbcNotif.getYSalto()
    Y.SHELL = Y.SHELL : "exit" : AbcNotif.getYSalto()
    Y.SHELL = Y.SHELL : "EOT"
    ...
    MENSAJE = ID.SMS.EMAIL:", Y.RETURNVAL: ":Y.RETURNVAL
    GOSUB BITACORA.ALT
...
*CAST20220907.F
    ...
    FOR X = 1 TO Y.NO.FIJOS
        Y.DATOS = Y.DATOS : Y.LIST.PARAMS<X>:'=':Y.LIST.VALUES<X>:','
    NEXT X
    ...
    FOR X = 1 TO Y.NO.CAMPOS
        ...
        END ELSE
            GOSUB OBTIENE.VALOR
        END
        Y.DATOS = Y.DATOS : Y.LIST.PARAMS<X>:'=':Y.VALOR.CAMPO:
        IF X NE Y.NO.CAMPOS THEN Y.DATOS = Y.DATOS : ','
    NEXT X
...
*Validamos que el tipo de notificaci�n est� configurado
...
*----------------------------------------------------------------------
    ...
    AbcNotif.setIdSmsEmailOri(ID.SMS.EMAIL.ORI)

    BEGIN CASE
        CASE Y.TIPO.EMAIL.REV EQ 'EMAIL.SPEI.DEV'
            AbcSpei.abcEnvEmailSpeiDev()
    END CASE
