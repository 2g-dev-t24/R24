...
$PACKAGE AbcSpei
*-----------------------------------------------------------------------------
    SUBROUTINE ABC.SPEI.PREPARA.REQUEST(Y.STRING.ORIGINAL, Y.RESPONSE)
*-----------------------------------------------------------------------------
...
**********
    ...
    Y.IDEMPRESA                 = FIELD(Y.DATOS,Y.SEP,2)
...
PROCESS:
********
    Y.STRING.ORIGINAL := Y.SEPJ : Y.CVEENTIDAD : Y.SEPJ
    Y.STRING.ORIGINAL := Y.IDEMPRESA : Y.SEPJ

    Y.CADENA.DATOS = ""
    AbcSpei.AbcSpeiGetdataRequest(Y.STRING.ORIGINAL, Y.CADENA.DATOS)
