* @ValidationCode : MjoxMDE5MjQyNzM6Q3AxMjUyOjE3NjE2MTYxNTQ4MTk6THVjYXNGZXJyYXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjI0X1NQMS4wOi0xOi0x
* @ValidationInfo : Timestamp         : 27 Oct 2025 22:49:14
...
*------------------------------------------------------------------------------
    ...
    BEGIN CASE
        ...
        CASE Y.TIPO.VALIDACION EQ 'CAJA' AND Y.VERSION.TT EQ ',ABC.2BR.EM.CASHINL'
            GOSUB VALIDA.LIMITE.DEPOSIT.CAJA

        CASE Y.TIPO.VALIDACION EQ 'CAJA' AND Y.VERSION.TT NE ',ABC.2BR.EM.CASHINL'
...
*-------------------------------------------------------------------------------
    ...
    IF R.EB.CONTRACT.BALANCES THEN
        Y.WB.BOVEDA = R.EB.CONTRACT.BALANCES<BF.ConBalanceUpdates.EbContractBalances.EcbWorkingBalance>
        Y.WB.BOVEDA = Y.WB.BOVEDA * (-1)
...
*-------------------------------------------------------------------------------
    R.EB.CONTRACT.BALANCES = ''
    Y.CTA.CAJA = EB.SystemTables.getRNew(TT.Contract.Teller.TeAccountTwo)

    EB.DataAccess.FRead(FN.EB.CONTRACT.BALANCES, Y.CTA.CAJA, R.EB.CONTRACT.BALANCES, F.EB.CONTRACT.BALANCES, Y.ERR.EBC)
    IF R.EB.CONTRACT.BALANCES THEN
        Y.WB.CAJA = R.EB.CONTRACT.BALANCES<BF.ConBalanceUpdates.EbContractBalances.EcbWorkingBalance>
        Y.WB.CAJA = Y.WB.CAJA * (-1)
...
*-------------------------------------------------------------------------------
    ...
    TIMESTAMP   = EB.SystemTables.getTimeStamp()
    ID.ABC.SMS.EMAIL.ENVIAR  = Y.NO.CUENTA:"-":TODAY:".":TIMESTAMP[1,2]
